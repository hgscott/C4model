...
#####
temp_seq$Delta = temp_seq$Ac - c3_temp_seq$A
par_seq$Delta = par_seq$Ac - c3_par_seq$A
co2_seq$Delta = co2_seq$Ac - c3_co2_seq$A
vpd_seq$Delta = vpd_seq$Ac - c3_vpd_seq$A

delta_temp <- ggplot(temp_seq, aes(tg_c, Delta)) + geom_line(size = 2) +
  labs(x = "Temperature (C)",
    y = expression(paste(Delta, A," (", mu * mol %.% m^{-2} %.% s^{-1}, ")"))) +
...
delta_co2 <- ggplot(co2_seq, aes(ca, Delta)) + geom_line(size = 2) +
  # lims(y = c(0, 60)) +
  labs(y = expression(paste(Delta, A," (", mu * mol %.% m^{-2} %.% s^{-1}, ")")),
       x = expression(paste('CO'[2], " (ppm)"))) + 
  theme(axis.title.x = element_text(size = 18),
        axis.title.y = element_text(size = 18),
        axis.text.x = element_text(size = 14),
...
# Delta A Multiple CO2 scenarios Line graphs (Overlayed)
#####
lowCO2_temp_seq <- C4model(tg_c = c(1:40), cao = 200)
highCO2_temp_seq <- C4model(tg_c = c(1:40), cao = 1000)


delta_temp_coseq <- ggplot(temp_seq, aes(tg_c, Delta, color = cao)) + geom_line(size = 2) +
  geom_line(data = lowCO2_temp_seq, aes(tg_c, Delta, color = cao), size = 2, linetype = "dashed") +
  geom_line(data = highCO2_temp_seq, aes(tg_c, Delta, color = cao), size = 2, linetype = "dotdash") +
  labs(x = "Temperature (C)",
       y = expression(paste(Delta, A," (", mu * mol %.% m^{-2} %.% s^{-1}, ")"))) +
  theme(axis.title.y = element_text(size = 18),
        axis.title.x = element_text(size = 18),
...
delta_vpd_coseq <- ggplot(vpd_seq, aes(vpd, Delta)) + geom_line(size = 2) +
  # lims(y = c(0, 60)) +
  labs(x = "Vapour Pressure Deficit (kPa)") +
  theme(axis.title.y = element_blank(),
        axis.title.x = element_text(size = 18),
...
ggarrange(delta_temp_coseq, delta_vpd_coseq, nrow = 1, common.legend = TRUE, legend="bottom")
#####
