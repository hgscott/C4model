...
pmodel = function(tg_c = 25, z = 0, vpdo = 1, cao = 400, 
  ...
  if(c3c4 == 'c3'){
    ...
    # Calculate Gamma star (needed in chi eqn)
    ...
    # calc vcmax
    vcmax <- Ap * ((cbs + kr * (1 + obs/ko)) / cbs) # Eqn. 2.47 # ADd in Os and ko
    Ac <- vcmax * ((cbs - gamma_star) / (kr * (1 + obs/ko) + cbs)) # Eqn. 2.4 # Add in Gamma star
